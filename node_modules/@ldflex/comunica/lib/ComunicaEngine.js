...
class ComunicaEngine {
  ...
  streamToAsyncIterable(readable) {
    ...
    let pendingError;
    readable.once('error', error => pendingError = error); // Return a asynchronous iterable
    ...
    function readNext(resolve, reject) {
      ...
      if (readable.ended) return resolve({
        done: true
      }); // Attach stream listeners

      readable.on('data', yieldValue);
      readable.on('end', finish);
      readable.on('error', finish); // Outputs the value through the iterable
